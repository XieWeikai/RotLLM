...
def quant_per_block_int8_kernel(Input, Output, Scale, L,
    ...
    x_int = x / scale
    x_int += 0.5 * tl.where(x_int >= 0, 1, -1)
    x_int = x_int.to(dtype)

    tl.store(output_ptrs, x_int, mask=offs_n[:, None] < L)
    tl.store(scale_ptrs, scale)

def per_block_int8(q, k, v, bits=8, BLKQ=128, BLKK=64, sm_scale=None, tensor_layout="HND"):
    """
    支持 bits = 8 或 16
    """
    ...
    q_int8 = torch.empty(q.shape, dtype=dtype, device=q.device)
    k_int8 = torch.empty(k.shape, dtype=dtype, device=k.device)
    v_int8 = torch.empty(v.shape, dtype=dtype, device=v.device)
    ...
    v_scale = torch.empty((b, h_kv, (kv_len + BLKK - 1) // BLKK, 1), device=q.device, dtype=torch.float32)
