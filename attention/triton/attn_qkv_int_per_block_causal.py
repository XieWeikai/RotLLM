...
def _attn_fwd_inner(acc, l_i, m_i, q, q_scale, kv_len,
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        qk = tl.dot((q * q_scale).to(tl.float16), (k * k_scale).to(tl.float16)).to(tl.float32)
        ...
        # p、v 量化（true）
        if tl_dtype == tl.int8:
            acc += (tl.dot(p_int.to(tl.float16), (v * v_scale).to(tl.float16)).to(tl.float32))
        elif tl_dtype == tl.int16:
            acc += (tl.dot(p_int.to(tl.float32), (v * v_scale).to(tl.float32)).to(tl.float32))
        else:
...
@triton.jit
def _attn_fwd(Q, K, V, Q_scale, K_scale, V_scale, Out, M,  
...
def forward(q, k, v, q_scale, k_scale, v_scale, M, bits=8, tensor_layout="HND", output_dtype=torch.float16):
    BLOCK_M = 128
    BLOCK_N = 64
    stage = 3
    ...
    _attn_fwd[grid](
        q, k, v, q_scale, k_scale, v_scale, o, M,
