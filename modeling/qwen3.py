...
class Qwen3MLPWithR4(nn.Module):
    def __init__(self, module: Qwen3MLP, R4):
        super().__init__()
...
def apply_R4_change_model(model, R4_list, local_rank=None):
    """
        Replace Qwen3MLP with Qwen3MLPWithR4
    """
    if local_rank is None:
        local_rank = 0
    layers = model.model.layers
    for i in tqdm(range(len(layers)), desc="Replace Qwen3MLP with Qwen3MLPWithR4", disable=not (local_rank == 0)):
        layer = layers[i]
        for name, module in layer.named_children():
            if isinstance(module, Qwen3MLP):
                # Take out the R4 of the corresponding layer from the list.
                R4 = R4_list[i]
                setattr(layer, name, Qwen3MLPWithR4(module, R4))