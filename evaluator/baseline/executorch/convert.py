...
ActivationQDQ_to_FakeQuantizer = {
    ...
    "self_attn.q_norm_input_qdq": "self_attn.q_proj.outActQuant",
    # "self_attn.q_proj_input_qdq": "self_attn.k_proj.actQuant",
    "self_attn.k_norm_input_qdq": "self_attn.k_proj.outActQuant",
    # "self_attn.q_proj_input_qdq": "self_attn.v_proj.actQuant",
    ...
    # "mlp.up_proj_input_qdq": "mlp.gate_proj.actQuant",
...
def rotllm_transform_to_executorch(model_rotllm, batch, model_args, R4):

    model_executorch = Qwen3ForCausalLM.from_pretrained(model_args.input_model, attn_implementation="eager").to(device=model_rotllm.device)
    untie_word_embeddings(model_executorch)

    print(model_executorch.model.device)
    ...
    for name, module_rotllm in to_replace_weight_quant:
        replace_module_by_name(model_executorch, name + ".weight_quant", module_rotllm)
    ...
    for name, module_exec in tqdm(model_executorch.named_modules(), desc="Rotllm transform to executorch(FakeQuantizer)"):
        ...
        replace_module_by_name(model_executorch, name, module_rotllm_copy)
    ...
    print("✅ Model_executorch init scale and zero_point ok!")
