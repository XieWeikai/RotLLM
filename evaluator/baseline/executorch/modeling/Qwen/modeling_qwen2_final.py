...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        self.q_proj = Qlinear_FakeQuantizer(config.hidden_size, config.num_attention_heads * self.head_dim, bias=True)
        self.k_proj = Qlinear_FakeQuantizer(config.hidden_size, config.num_key_value_heads * self.head_dim, bias=True)
        self.v_proj = Qlinear_FakeQuantizer(config.hidden_size, config.num_key_value_heads * self.head_dim, bias=True)
        self.o_proj = Qlinear_FakeQuantizer(config.num_attention_heads * self.head_dim, config.hidden_size, bias=False)
