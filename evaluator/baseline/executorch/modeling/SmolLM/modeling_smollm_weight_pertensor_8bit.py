...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        self.gate_proj = QLinearW8_PerTensorSym(
            self.hidden_size, self.intermediate_size, bias=config.mlp_bias
        )
        self.up_proj = QLinearW8_PerTensorSym(
            self.hidden_size, self.intermediate_size, bias=config.mlp_bias
        )
        self.down_proj = QLinearW8_PerTensorSym(
            self.intermediate_size, self.hidden_size, bias=config.mlp_bias
        )

        # QDQ
        self.up_proj_input_qdq = ActivationQDQ(bits=act_bits)
        self.up_proj_output_qdq = ActivationQDQ(bits=act_bits)
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.q_proj = QLinearW8_PerTensorSym(
            config.hidden_size,
            config.num_attention_heads * self.head_dim,
            bias=config.attention_bias,
        )
        self.k_proj = QLinearW8_PerTensorSym(
            config.hidden_size,
            config.num_key_value_heads * self.head_dim,
            bias=config.attention_bias,
        )
        self.v_proj = QLinearW8_PerTensorSym(
            config.hidden_size,
            config.num_key_value_heads * self.head_dim,
            bias=config.attention_bias,
        )
        self.o_proj = QLinearW8_PerTensorSym(
            config.num_attention_heads * self.head_dim,
            config.hidden_size,
            bias=config.attention_bias,
        )
        ...
        self.q_proj_output_qdq = ActivationQDQ(bits=act_bits)
        self.k_proj_output_qdq = ActivationQDQ(bits=act_bits)
