...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        gated_activation = gate_result * up_result
        if self.R4 is not None:
            gated_activation_dtype = gated_activation.dtype
            gated_activation_device = gated_activation.device
            gated_activation = (gated_activation.to(dtype = self.R4.weight.dtype) @ self.R4.weight.to(gated_activation_device)).to(dtype = gated_activation_dtype)

        o = self.down_proj_input_qdq(gated_activation)
        o = self.down_proj(o)
        return o
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.q_proj = Qlinear_FakeQuantizer(config.hidden_size, config.num_attention_heads * self.head_dim, bias=config.attention_bias)
        self.k_proj = Qlinear_FakeQuantizer(config.hidden_size, config.num_key_value_heads * self.head_dim, bias=config.attention_bias)
        self.v_proj = Qlinear_FakeQuantizer(config.hidden_size, config.num_key_value_heads * self.head_dim, bias=config.attention_bias)
        self.o_proj = Qlinear_FakeQuantizer(config.num_attention_heads * self.head_dim, config.hidden_size, bias=config.attention_bias)
        
        # QDQ
        self.q_proj_input_qdq = ActivationQDQ(bits=act_bits)
        ...
        self.q_rope_mul_1_output_qdq = ActivationQDQ(bits=act_bits)
        self.q_rope_add_0_output_qdq = ActivationQDQ(bits=act_bits)
        ...
        self.v_proj_output_qdq = ActivationQDQ(bits=8)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        query_states = self.q_proj_output_qdq(self.q_proj(hidden_states)).view(hidden_shape).transpose(1, 2)
...
class LlamaDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.o_proj_output_qdq = ActivationQDQ(bits=8)
...
class LlamaModel(LlamaPreTrainedModel):
    def __init__(self, config: LlamaConfig):
        ...
        self.sin_embedding_input_qdq = ActivationQDQ(bits=act_bits)
        self.cos_embedding_input_qdq = ActivationQDQ(bits=act_bits)
        self.norm_input_qdq = ActivationQDQ(bits=act_bits)
...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.lm_head_input_qdq = ActivationQDQ(bits=act_bits)
        self.lm_head_output_qdq = ActivationQDQ(bits=act_bits)
