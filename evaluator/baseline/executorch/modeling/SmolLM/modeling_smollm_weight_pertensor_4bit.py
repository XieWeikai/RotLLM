...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        self.gate_proj = QLinearW4_PerTensorSym(
            self.hidden_size, self.intermediate_size, bias=config.mlp_bias
        )
        self.up_proj = QLinearW4_PerTensorSym(
            self.hidden_size, self.intermediate_size, bias=config.mlp_bias
        )
        self.down_proj = QLinearW4_PerTensorSym(
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.q_proj = QLinearW4_PerTensorSym(
            config.hidden_size,
            config.num_attention_heads * self.head_dim,
            bias=config.attention_bias,
        )
        self.k_proj = QLinearW4_PerTensorSym(
            config.hidden_size,
            config.num_key_value_heads * self.head_dim,
            bias=config.attention_bias,
        )
        self.v_proj = QLinearW4_PerTensorSym(
            config.hidden_size,
            config.num_key_value_heads * self.head_dim,
            bias=config.attention_bias,
        )
        self.o_proj = QLinearW4_PerTensorSym(
            config.num_attention_heads * self.head_dim,
            config.hidden_size,
            bias=config.attention_bias,
        )
